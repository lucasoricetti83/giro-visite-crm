...
/* ===== BOTTONI STREAMLIT (adattivi light/dark) ===== */
...
/* Bottone PRIMARY — tono invertito rispetto al tema */
...
hr {
    ...
    border-top: 0.5px solid rgba(128,128,128,0.22) !important;
}

/* ===== CONTAINER BORDER (st.container(border=True)) ===== */
[data-testid="stVerticalBlockBorderWrapper"] {
    border-radius: 12px !important;
    border: 0.5px solid rgba(128,128,128,0.25) !important;
}
...
/* ===== SLIDER ===== */
.stSlider > div > div > div > div { background-color: var(--primary-color, #2196f3) !important; }

/* ===== CHECKBOX / TOGGLE ===== */
.stCheckbox, .stToggle { font-size: 14px; }

/* ===== HTML CUSTOM CARDS (adattivi light/dark mode) ===== */
/* Usiamo le variabili di Streamlit quando disponibili, con fallback safe */
...
.gv-date-big { font-size: 18px; font-weight: 500; color: var(--text-color, #1d1d1f); line-height: 1.2; }
.gv-date-sub { font-size: 12px; color: rgba(128,128,128,0.9); margin-top: 2px; }
...
/* Numeri colorati — sfondo tenue + testo carico, leggibile in entrambi i temi */
.gv-num-red { background: rgba(244,67,54,0.18); color: #f44336; }
.gv-num-orange { background: rgba(255,152,0,0.18); color: #ff9800; }
.gv-num-gray { background: rgba(128,128,128,0.18); color: inherit; opacity: 0.85; }
.gv-num-blue { background: rgba(33,150,243,0.18); color: #2196f3; }
.gv-num-green { background: rgba(76,175,80,0.22); color: #4caf50; }

.gv-info { flex: 1; min-width: 0; }
.gv-line1 {
    display: flex; align-items: baseline; justify-content: space-between; gap: 8px;
}
/* NOME CLIENTE: usa il colore testo primario del tema corrente → bianco in dark, nero in light */
.gv-name {
    font-size: 15px; font-weight: 500;
    color: var(--text-color, #1d1d1f);
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.gv-time { font-size: 12px; color: rgba(128,128,128,0.95); flex-shrink: 0; }
.gv-line2 {
    font-size: 12px; color: rgba(128,128,128,0.95);
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    margin-top: 1px;
}
.gv-badge-red { color: #f44336; font-weight: 500; }
.gv-badge-orange { color: #ff9800; font-weight: 500; }
...
.gv-row-clickable:hover .gv-name {
    color: #2196f3;
}

/* === RIGA CLIENTE VISITATO (spunta) === */
.gv-row-done {
    opacity: 0.75;
}
.gv-row-done .gv-name {
...
def fetch_config():
    ...
    Auto-consolida eventuali righe duplicate per lo stesso utente (residuo di un vecchio bug
    per cui, se il salvataggio non "vedeva" la riga esistente, ne creava una nuova invece di
    aggiornarla — con più righe presenti, una lettura senza ordinamento poteva pescarne una
    vecchia/obsoleta invece dell'ultima salvata). Ordiniamo per id decrescente e teniamo solo
    la più recente, ripulendo le altre.
    """
    try:
        user_id = get_user_id()
        if not user_id:
        ...
        try:
            response = supabase.table('config_utente').select('*').eq('user_id', user_id).order('id', desc=True).execute()
        except Exception:
        ...
        righe = response.data
        config_attuale = righe[0]

        if len(righe) > 1:
            try:
                for _riga_vecchia in righe[1:]:
                    if _riga_vecchia.get('id') is not None:
                        supabase.table('config_utente').delete().eq('id', _riga_vecchia['id']).execute()
            except Exception:
                pass  # pulizia best-effort: non deve mai bloccare la lettura della config

        return config_attuale
    except Exception as e:
        return None
    ...
    try:
        ...
        for tentativo in range(max_tentativi):
            try:
                ...
                if existing:
                    _resp_save_cfg = supabase.table('config_utente').update(config_data).eq('user_id', user_id).execute()
                else:
                    _resp_save_cfg = supabase.table('config_utente').insert(config_data).execute()

                if not _resp_save_cfg.data:
                    st.error(
                        "❌ Salvataggio configurazione non riuscito: nessuna riga modificata "
                        "(probabile blocco permessi/RLS). Controlla il secret SUPABASE_SERVICE_ROLE_KEY "
                        "su Streamlit Cloud, poi riprova."
                    )
                    return False
...
def calcola_agenda_settimanale(df, config, esclusi=[], settimana_offset=0, variante=0, override_ultima_visita=None, giorni_non_disponibili=None, spostamenti_cliente=None):
    ...
    # Prima: gestisci i giorni con appuntamento (l'app è il baricentro)
    
    # ========================================
    # 5b. GIORNI CON APPUNTAMENTO → BARICENTRO
    # ========================================
    giorni_con_app = set()
    nomi_usati_da_app = set()
    risultati_app = {}
    ...
    for g_app in sorted(app_per_giorno.keys()):
        ...
        n_giorni_senza_app -= 1
        
        # Baricentro degli appuntamenti del giorno
        app_lat = sum(t['lat'] for t in tappe_app) / len(tappe_app)
        app_lon = sum(t['lon'] for t in tappe_app) / len(tappe_app)
        
        slot = max_visite - len(tappe_app)
        
        # Prendi dal pool INTERO i clienti più vicini all'appuntamento
        candidati = [c for c in pool if c['nome'] not in nomi_usati_da_app]
        candidati.sort(key=lambda c: (haversine(c['lat'], c['lon'], app_lat, app_lon), -c['urgenza']))
        selezionati = candidati[:slot]
        
        for c in selezionati:
            nomi_usati_da_app.add(c['nome'])
...
""", unsafe_allow_html=True)
    ...
    # --- TAB: DASHBOARD ---
        ...
        if critici:
            with st.expander(f"🚨 Clienti Critici ({len(critici)})", expanded=len(critici) > 0):
                for c in critici[:10]:
        ...
        # === STORICO VISITE (in expander) ===
        with st.expander("📅 Storico Visite", expanded=False):
            ...
            if not df.empty and 'ultima_visita' in df.columns:
                ...
                if not df_storico.empty:
                    ...
                    if not df_visitati.empty:
                        ...
                        st.caption(f"📋 Clienti visitati ({len(df_visitati)})")
                        for _, row in df_visitati.iterrows():
                            with st.container(border=True):
                                ...
                                colori_stato = {'CLIENTE ATTIVO': '🟢', 'CLIENTE NUOVO': '🔵', 'CLIENTE POSSIBILE': '🟡', 'CLIENTE PROBABILE': '🟠'}
        ...
        # === STATISTICHE PER STATO (in expander) ===
        with st.expander("📈 Statistiche per Stato Cliente", expanded=False):
        ...
        if giorni_attivi:
            ...
            with st.expander("ℹ️ Come funziona l'ottimizzazione"):
                ...
                **L'algoritmo v10 considera:**
                - 📌 **Appuntamento = baricentro**: le visite del giorno ruotano intorno all'appuntamento
                - 📍 **Zone compatte**: K-Means raggruppa i clienti in zone geografiche vicine
                - 🛣️ **Distanze reali**: Google Maps calcola tempi su strade vere
                - 🔄 **Ordine ottimale**: TSP Held-Karp trova il giro più corto
                - ⏰ **Pool 10 giorni**: clienti scaduti o in scadenza entro 10gg
        ...
        if not df.empty:
            ...
            if scelto:
                ...
                with st.container(border=True):
                    ...
                    with col_vis4:
                        st.write("**🚗 Nel giro**")
                        # Spunta rapida: cambia stato e salva subito, senza un pulsante separato
                        nuovo_stato_vis = st.checkbox(
                            "✅ Nel giro" if is_nel_giro else "❌ Fuori dal giro",
                            value=is_nel_giro,
                            key=f"chk_giro_{cliente['id']}",
                            help="Spunta per includere il cliente nel giro visite, togli la spunta per escluderlo"
                        )
                        if nuovo_stato_vis != is_nel_giro:
            ...
            # (spostata qui in fondo: all'apertura della tab si vede
            # subito la scheda del cliente cercato, non questo pannello)
            # =============================================
            st.divider()
